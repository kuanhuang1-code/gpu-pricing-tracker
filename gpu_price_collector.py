...
def run_scheduler(interval_hours=168):
    import schedule
    import time

    print(f"Starting GPU price collection scheduler (every {interval_hours} hours)")
    print(f"Next collection will run at start, then every {interval_hours}h")
    print("Press Ctrl+C to stop.\n")

    # Run once immediately at startup
    collect()

    schedule.every(interval_hours).hours.do(collect)
